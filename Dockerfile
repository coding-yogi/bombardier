...
RUN apk add --update --no-cache g++ gcc libxslt-dev libgcc openssl-dev pkgconfig 
RUN RUSTFLAGS="-C target-feature=-crt-static" cargo build --release --target=x86_64-unknown-linux-musl

FROM alpine:latest
RUN apk add --update --no-cache libxslt-dev libgcc openssl-dev pkgconfig ca-certificates
WORKDIR /home
